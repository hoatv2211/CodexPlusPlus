...
import { localizeCurrentText } from "@/lib/i18n";
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, ...props }, ref) => {
    ...
    const localizedChildren = React.Children.map(props.children, (child) =>
      typeof child === "string" ? localizeCurrentText(child) : child,
    );
    return <Comp className={cn(buttonVariants({ variant, size, className }))} ref={ref} {...props}>{localizedChildren}</Comp>;
  },
);
